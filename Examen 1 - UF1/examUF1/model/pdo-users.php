...
function userAdmin($userId) {
    try {
        ...
        $statement = $connexio->prepare('SELECT admin FROM users WHERE id = :userId');
...
function showUsers() {
    try {
        ...
        $statement = $connexio->prepare('SELECT nickname FROM users');
...
function novaContrasenya($userId, $pswd) {
    try {
        $connection = getConnection();

    $statement = $connection->prepare('UPDATE password:pswd FROM users WHERE id = :userId');

    $statement->bindParam('paswd', $pswd, PDO::PARAM_INT);
    $statement->bindParam('userId', $userId, PDO::PARAM_INT);
...
function getEmail($userId) {
    ...
    $statement = $connection->prepare('SELECT email FROM users WHERE id = :userId');
